...
const regexFromMyArray = new RegExp(types.join('|'), 'gi')
...
const categorizePullRequests = (pullRequests, config) => {
  ...
  const filterExcludedPullRequests = pullRequest => {
    const labels = pullRequest.title.match(regexFromMyArray) || []
  ...
  const filterUncategorizedPullRequests = pullRequest => {
    ...
    if (
      labels.length === 0 ||
      !labels.some(label => allCategoryLabels.includes(label))
    ) {
